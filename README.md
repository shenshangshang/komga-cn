...
- “网站地址”可配置为公网域名、IP 或反向代理子路径，生成的邀请链接将使用该地址；留空则使用当前访问地址。
- 新注册用户默认不能看到任何媒体库；管理员可在用户限制中分配可见媒体库。
- 普通用户权限新增“创建媒体库”和“上传漫画压缩包”，与下载、页面浏览、Kobo/KOReader 同步权限独立配置。
- 具备上传权限的用户可选择其可见媒体库，将 CBZ、CBR、ZIP、RAR 或 7Z 文件添加到现有非单本系列，或在该媒体库中新建系列；单文件最大 1 GiB。
- 管理员可在服务器设置中指定一个或多个“普通用户建库允许根目录”。具备创建媒体库权限的普通用户只会看到这些根目录及其子目录，并只能选择根目录下的子目录建库。
