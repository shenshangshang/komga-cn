...
class HistoricalEventTest {
  ...
  fun `book events retain a readable book name`() {
    ...
    val deleted = HistoricalEvent.BookFileDeleted(book, "deleted", "小飞鼠合集")
    val imported = HistoricalEvent.BookImported(book, series, book.path, false)
    val analyzed = HistoricalEvent.BookAnalyzed(book, "小飞鼠合集")

    assertThat(deleted.properties["bookName"]).isEqualTo("小飞鼠单话")
    assertThat(deleted.properties["seriesName"]).isEqualTo("小飞鼠合集")
    ...
    assertThat(analyzed.properties["seriesName"]).isEqualTo("小飞鼠合集")
  }
}
