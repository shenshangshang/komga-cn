...
- 修复未登录用户打开邀请注册链接时被重定向、邀请码丢失的问题。
- 邀请码提交前可公开预校验，并自动清理复制时带入的首尾空格。
- 服务器设置新增“网站地址”，邀请链接可固定使用公网域名、IP 或反向代理子路径。
- 漫画压缩包上传新增媒体库选择，可添加到现有系列或在授权媒体库中安全创建新系列。
- 管理员可配置普通用户建库允许根目录；普通用户仅能浏览允许根及其子目录，并只能在严格子目录中创建媒体库。
